...
$basvur = "Update arizabildirimi set AdSoyad = '$AdSoyad',SeriNo = '$SeriNo',MarkaModel = '$MarkaModel',ArizaAciklamasi = '$ArizaAciklamasi' Where id = $id";
?>
        ...
        <div class="alert bildirim border-0 rounded-0 m-0 mt-3 text-center fs-4 text-white w-100" role="alert">
          ...
          if ($AdSoyad != "" && $SeriNo != "" && $MarkaModel != "" && $ArizaAciklamasi != "") {
            if ($baglan->query($basvur)) {
              echo $baglan->affected_rows . " Ariza Guncellendi";
            }
          }
          $baglan->close();
          ?>
